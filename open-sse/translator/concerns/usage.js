...
const USAGE_EXTRACTORS = {
  ...
  commandcode(raw) {
    ...
    // DeepSeek prefix-cache telemetry, live-probe verified (2026-08-10). CommandCode maps
    // cache reads into several nested shapes; inputTokens already includes the cached portion,
    // so we only need to surface the hit count. Priority: mapped field → nested details →
    // DeepSeek-native raw → provider metadata.
    const cached =
      n(raw.cachedInputTokens) || n(raw.inputTokenDetails?.cacheReadTokens) ||
      n(raw.raw?.prompt_cache_hit_tokens) || n(raw.providerMetadata?.deepseek?.promptCacheHitTokens);
