...
function cacheCreationTokensFrom(tokens) {
  const value = tokens?.cache_creation_input_tokens ?? tokens?.prompt_tokens_details?.cache_creation_tokens;
  const numeric = Number(value);
  return Number.isFinite(numeric) && numeric > 0 ? numeric : 0;
}

function addCacheRatios(target) {
  if (!target || typeof target !== "object") return;
  const promptTokens = Number(target.promptTokens) || 0;
  const cachedTokens = Number(target.cachedTokens) || 0;
  target.cacheCreationTokens = Number(target.cacheCreationTokens) || 0;
  target.cacheHitRatio = promptTokens > 0 ? cachedTokens / promptTokens : 0;
}

function finalizeCacheMetrics(stats) {
  stats.totalCacheHitRatio = stats.totalPromptTokens > 0
    ? stats.totalCachedTokens / stats.totalPromptTokens
    : 0;
  for (const mapName of ["byProvider", "byModel", "byAccount", "byApiKey", "byEndpoint"]) {
    for (const value of Object.values(stats[mapName] || {})) addCacheRatios(value);
  }
  return stats;
}
...
function aggregateEntryToDay(day, entry) {
  ...
  const cacheCreationTokens = cacheCreationTokensFrom(entry.tokens);
...
export async function getUsageStats(period = "all") {
  ...
  const recentRequests = recentRows
    .map((r) => {
      ...
      return {
        ...
        cacheCreationTokens: cacheCreationTokensFrom(t),
        status: r.status || "ok",
      };
    })
    .filter((e) => {
  ...
  if (useDailySummary) {
    ...
    for (const r of filtered) {
      ...
      const cacheCreationTokens = cacheCreationTokensFrom(tokens);
  ...
  return finalizeCacheMetrics(stats);
}
