...
function isLoopbackHostname(h) {
  ...
  let name = h.replace(/^\[|\]$/g, "").toLowerCase();
  // Strip port only if it's an IPv4 address or an IPv6 address with a port
  // A simple way to strip port from the end safely:
  if (name.includes("]:")) {
    name = name.split("]:")[0].replace("[", "");
  } else if (name.includes(":") && !name.includes("::")) {
    name = name.split(":")[0];
  } else if (name.match(/:\\d+$/)) {
    // IPv4 with port, or IPv6 with port and no brackets (rare but possible)
    name = name.replace(/:\\d+$/, "");
  }
  
  if (name === "::ffff:127.0.0.1") return true;
  if (name === "0:0:0:0:0:0:0:1") return true;
  return LOOPBACK_HOSTS.has(name) || name.includes("localhost") || name.includes("127.0.0.1") || name.includes("::1");
}
