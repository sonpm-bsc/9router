...
describe("toOpenAIUsage", () => {
  ...
  it("commandcode: surfaces cacheReadTokens as cached_tokens (probe-verified shape)", () => {
    // Raw finish-step usage captured live from api.commandcode.ai (2026-08-10 probe):
    // inputTokens ALREADY includes the cached portion; the hit count lives in nested fields.
    const u = toOpenAIUsage(
      { inputTokens: 6075, outputTokens: 32, totalTokens: 6107,
        cachedInputTokens: 6016, inputTokenDetails: { noCacheTokens: 59, cacheReadTokens: 6016 },
        raw: { prompt_cache_hit_tokens: 6016, prompt_cache_miss_tokens: 59 },
        providerMetadata: { deepseek: { promptCacheHitTokens: 6016, promptCacheMissTokens: 59 } } },
      "commandcode"
    );
    expect(u.prompt_tokens).toBe(6075);
    expect(u.completion_tokens).toBe(32);
    expect(u.total_tokens).toBe(6107);
    expect(u.prompt_tokens_details.cached_tokens).toBe(6016);
  });

  it("commandcode: cached_tokens absent when cacheRead is 0", () => {
    const u = toOpenAIUsage(
      { inputTokens: 3304, outputTokens: 32, totalTokens: 3336, cachedInputTokens: 0,
        inputTokenDetails: { noCacheTokens: 3304, cacheReadTokens: 0 },
        raw: { prompt_cache_hit_tokens: 0, prompt_cache_miss_tokens: 3304 } },
      "commandcode"
    );
    expect(u.prompt_tokens).toBe(3304);
    expect(u.prompt_tokens_details).toBeUndefined();
  });
